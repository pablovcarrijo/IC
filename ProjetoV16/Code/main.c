...
int main(int argc, char *argv[])
{
    ...
    int mapViewSize = 600;
    int largestMapDimension = rows > cols ? rows : cols;
    float cellSize = (float)mapViewSize / largestMapDimension;
    float cellWidth = cellSize;
    float cellHeight = cellSize;
    int screenWidth = mapViewSize;
    int screenHeight = mapViewSize + PANEL_HEIGHT;
    int panelY = mapViewSize;
    ...
    while (!WindowShouldClose() && cycle < maxCycles)
    {
        ...
        if (frameCounter % 12 == 0)
        {
            ...
            for (int i = 0; i < quantityRobots; i++)
            {
                com_ReadPool(&robots[i], &pool, &realMap, rows, cols);
            }
