...
void init_robots(int id, int rows, int columns, Robot *robot)
{
    ...
    robot->detectionRadius = DETECTION_RADIUS;

    init_queue(&robot->pathMemory, 50);
...
void robot_move(int rows, int columns, Robot *robot)
{
    ...
    for (i = -DETECTION_RADIUS; i <= DETECTION_RADIUS; i++)
    {
        for (j = -DETECTION_RADIUS; j <= DETECTION_RADIUS; j++)
        {
            if (j == 0 && i == 0)
                continue;
            ...
            if (neighbor_i >= 0 && neighbor_j >= 0 && neighbor_i < rows && neighbor_j < columns)
            {
                if (robot->mapRobot[neighbor_i][neighbor_j] != CELL_WALL)
                {
                    ...
                    visitedCell.pos.x = neighbor_i;
                    visitedCell.pos.y = neighbor_j;
                    visitedCell.pheromoneValue = robot->pheromoneGrade[neighbor_i][neighbor_j].pheromoneValue;
                    enqueue_cell(&robot->pathMemory, visitedCell);
    ...
    visitedCell.pos.x = new_i;
    visitedCell.pos.y = new_j;
