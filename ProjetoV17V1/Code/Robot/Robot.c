...
int wall_is_visible(Position origin, int targetRow, int targetCol, RealMap *realMap)
{
    int currentRow = origin.x;
    int currentCol = origin.y;
    int deltaRow = abs(targetRow - currentRow);
    int deltaCol = abs(targetCol - currentCol);
    int stepRow = currentRow < targetRow ? 1 : -1;
    int stepCol = currentCol < targetCol ? 1 : -1;
    int error = deltaRow - deltaCol;
    ...
    while (currentRow != targetRow || currentCol != targetCol)
    {
        ...
        if ((currentRow != targetRow || currentCol != targetCol) &&
            realMap->value[currentRow][currentCol] == CELL_WALL)
        {
            return 0;
        }
    }
...
void map_visible_walls(int rows, int columns, Robot *robot, RealMap *realMap)
{
    for (int rowOffset = -WALL_MAPPING_RADIUS; rowOffset <= WALL_MAPPING_RADIUS; rowOffset++)
    {
        for (int colOffset = -WALL_MAPPING_RADIUS; colOffset <= WALL_MAPPING_RADIUS; colOffset++)
        {
            int row = robot->pos.x + rowOffset;
            int col = robot->pos.y + colOffset;
            ...
            if (realMap->value[row][col] == CELL_WALL &&
                wall_is_visible(robot->pos, row, col, realMap))
            {
                robot->mapRobot[row][col] = CELL_WALL;
            }
        }
    }
}
...
int init_robots(int id, int rows, int columns, Robot *robot)
{
    ...
    robot->id = id;
    robot->stepsSinceLastQueueWrite = 0;
...
void place_robot_random(int rows, int columns, int quantityPlaced, Robot *robots, Robot *robot, RealMap *realMap)
{
    ...
    map_visible_walls(rows, columns, robot, realMap);
...
void robot_move(int rows, int columns, int id, int quantityRobots, Robot *robot, Robot *robots, Pool *pool, RealMap *realMap)
{
        ...
        !verify_position_occuped(robots, quantityRobots, id, new_i, new_j))
    {
        ...
        robot->pos.x = new_i;
        robot->pos.y = new_j;
        robot->stepsSinceLastQueueWrite++;
    }
}
