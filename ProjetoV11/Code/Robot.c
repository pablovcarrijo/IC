...
void place_robot_random(int rows, int columns, int quantityPlaced, Robot *robot, RealMap *realMap)
{
    int x, y;
    ...
    do
    {
        x = (rand() % (rows - 2)) + 1;
        y = (rand() % (columns - 2)) + 1;
    } while (realMap->value[x][y] == CELL_WALL || verify_position_occupied(robot, quantityPlaced, x, y));
